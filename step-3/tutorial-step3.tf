#
# Add API-Key for Authentication, and an usage plan
#
...
resource "aws_api_gateway_usage_plan" "sap_kafka_gateway_usageplan" {
  name = "sap-kafka-gateway_usage_plan"

  api_stages {
    api_id = aws_api_gateway_rest_api.api.id
    stage  = aws_api_gateway_deployment.aws_api_gateway_deployment.stage_name
  }
}

resource "aws_api_gateway_api_key" "sap_kafka_key" {
  name = "test-api-key"
}

resource "aws_api_gateway_usage_plan_key" "main" {
  key_id        = aws_api_gateway_api_key.sap_kafka_key.id
  key_type      = "API_KEY"
  usage_plan_id = aws_api_gateway_usage_plan.sap_kafka_gateway_usageplan.id
}
